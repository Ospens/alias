...
const WordSetsScreen: FC<INavigatorProps<"WordSets">> = observer(({ navigation }) => {
  ...
  const bottomPanel = useMemo(() => {
    return (
      <>
        <RectangleButton isSquare onPress={navigation.goBack} style={styles.backButton}>
          <ArrowIcon />
        </RectangleButton>
        <RectangleButton
  ...
  }, [checkedGroups.length, gotoGame, locale.actions.next, navigation.goBack]);
  ...
  return (
    <MainLayout style={styles.container} bottomPanel={bottomPanel}>
      <FlatList
        ...
        renderItem={({ item: set }) => (
          <WordSet key={set.id} set={set} containerStyle={styles.wordContainer} />
        )}
      />
    </MainLayout>
  );
